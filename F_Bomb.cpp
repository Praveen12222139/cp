...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        max_heap.push({v1[i], i});
    }

    ll ans = 0;
    unordered_map<ll, ll> latest_values;
    while (k--) {
       
        while (!max_heap.empty()) {
            auto [maxi, index] = max_heap.top();
            max_heap.pop();
           
            if (latest_values.find(index) != latest_values.end() && latest_values[index] != maxi) {
                continue;
            }
            ans += maxi;
            ll new_value = max(0LL, v1[index] - v2[index]);
            v1[index] = new_value;
            latest_values[index] = new_value;
            max_heap.push({new_value, index});
            break;
        }
    }
