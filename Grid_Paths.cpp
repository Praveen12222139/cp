...
int ans(const vector<vector<char>>& grid, int i, int j, vector<vector<int>>& dp) {
        if (i < 0 || j < 0) return 0;
    ...
    long long left = ans(grid, i, j - 1, dp);
    long long up = ans(grid, i - 1, j, dp);
